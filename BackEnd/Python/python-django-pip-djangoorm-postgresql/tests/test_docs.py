...
class DocsTests(SimpleTestCase):
    def test_openapi_endpoint_returns_document(self):
        ...
        self.assertIn("/api/v1/cart", payload["paths"])
        self.assertIn("/api/v1/cart/{item_id}", payload["paths"])
        ...
        self.assertIn("/api/v1/products/{product_id}/price-history", payload["paths"])
        self.assertIn("/api/v1/prices/{price_id}", payload["paths"])
        self.assertIn("/api/v1/price-alerts", payload["paths"])
        self.assertIn("/api/v1/price-alerts/{alert_id}", payload["paths"])
        ...
        self.assertIn("CartItem", payload["components"]["schemas"])
        ...
        self.assertIn("ProductPriceHistoryResponse", payload["components"]["schemas"])
        self.assertIn("PriceAlert", payload["components"]["schemas"])
