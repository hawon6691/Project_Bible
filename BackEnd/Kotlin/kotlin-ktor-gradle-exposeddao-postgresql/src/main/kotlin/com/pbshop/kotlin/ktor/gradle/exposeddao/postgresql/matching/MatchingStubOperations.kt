...
fun matchingOperations(): List<StubOperation> =
    listOf(
        ...
        endpoint(HttpMethod.Patch, "/matching/{id}/approve", "Matching", "Approve product mapping", roles = setOf(PbRole.ADMIN)) { call ->
            StubResponse(
                data = mapOf(
        ...
        endpoint(HttpMethod.Patch, "/matching/{id}/reject", "Matching", "Reject product mapping", roles = setOf(PbRole.ADMIN)) { call ->
            StubResponse(
                data = mapOf(
                    "id" to call.pathParam("id", "1"),
                    "status" to "REJECTED",
                    "reason" to "Not a valid match",
                )
            )
        },
        ...
        endpoint(HttpMethod.Get, "/matching/stats", "Matching", "Product mapping stats", roles = setOf(PbRole.ADMIN)) {
            StubResponse(data = mapOf("approved" to 120, "pending" to 8, "rejected" to 5, "total" to 133))
        },
    )
