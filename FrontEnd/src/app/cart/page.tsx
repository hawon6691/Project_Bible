...
export default function CartPage() {
  ...
  const columns = [
    { title: '상품', key: 'product', render: (_: any, r: CartItem) => (<Space><div style={{ width: 60, height: 60, background: '#f5f5f5', borderRadius: 4, overflow: 'hidden' }}>{r.thumbnailUrl && <img src={r.thumbnailUrl} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />}</div><div><Text strong style={{ cursor: 'pointer' }} onClick={() => router.push(ROUTES.PRODUCT_DETAIL(r.productId))}>{r.productName}</Text>{r.selectedOptions && <div><Text type="secondary" style={{ fontSize: 12 }}>{r.selectedOptions}</Text></div>}<div><Text type="secondary" style={{ fontSize: 12 }}>판매자: {r.sellerName}</Text></div></div></Space>) },
